...
# =============================================================================
# TwoStageDetector - Two-stage detection with rich features
# =============================================================================


class TwoStageDetector:
    """
    Two-stage X-ray object detector with improved precision.

    Stage 1: Object vs Background binary classification
    Stage 2: 6-way object class classification (only on Stage 1 positives)

    Key features:
    - Aggressive multi-method proposal generation (8 methods)
    - Rich feature extraction (HOG, edges, texture, shape - 93 features)
    - Balanced reject class sampling
    - Per-class size/shape constraints learned from training data
    ...
    def _extract_hog_features(self, gray: np.ndarray) -> list:
    ...
    def _generate_proposals(self, img: np.ndarray) -> List[Tuple[int, int, int, int]]:
    ...
    def _extract_contours(self, binary: np.ndarray, max_area: float) -> List[Tuple]:
    ...
    def _multi_threshold_intensity(
        self, gray: np.ndarray, max_area: float
    ) -> List[Tuple]:
        proposals = []
    ...
    def _aggressive_edge_detection(
        self, gray: np.ndarray, max_area: float
    ) -> List[Tuple]:
        proposals = []
    ...
    def _mser_detection(self, gray: np.ndarray, max_area: float) -> List[Tuple]:
        proposals = []
        try:
    ...
    def _inverted_intensity_detection(
        self, gray: np.ndarray, max_area: float
    ) -> List[Tuple]:
        proposals = []
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    ...
    def _apply_proposal_nms(self, boxes: List[Tuple]) -> List[Tuple]:
    ...
    def train(self, dataset, validate: bool = True):
        ...
        print(f"Undersampled background: {len(X_background)} -> {n_background}")
    ...
    def _class_specific_nms(
        self, predictions: List[List], img_w: int, img_h: int
    ) -> List[List]:
